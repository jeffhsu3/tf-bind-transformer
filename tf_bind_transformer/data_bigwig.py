...
from tf_bind_transformer.data import (
    FactorProteinDataset,
    ContextDataset,
    cast_list,
    filter_df_by_tfactor_fastas,
)
from tf_bind_transformer.data import (
    pl_isin,
    pl_notin,
    fetch_experiments_index,
    parse_exp_target_cell,
    read_bed,
    cycle,
    filter_by_col_isin,
)
...
def chip_atlas_add_experiment_target_cell(
    df, col_target="column_4", col_cell_type="column_5"
):
    df = df.clone()
...
class BigWigDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.factor_ds = FactorProteinDataset(
            factor_fasta_folder, species_priority=factor_species_priority
        )
        ...
        print(
            f'{", ".join(only_ref)} - {len(filtered_out_exp_ids)} experiments filtered out by lack of transcription factor fastas',
            filtered_out_exp_ids,
        )
        ...
        self.bigwigs = [
            pyBigWig.open(str(bigwig_folder / f"{str(i)}.bw"))
            for i in self.annot.get_column("column_1")
        ]
...
class BigWigTracksOnlyDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # filtering loci by chromosomes
        # as well as training or validation
        # :TODO switch to only use K-FOLD Cross-validation
